...
namespace myslam
{
  // Frame::Frame()
   ...
   // {
    ...
    //: id_(id), time_stamp_(time_stamp), T_c_w(T_c_w), camera_(camera), color_(color), depth_(depth)
    //{
...
bool Frame::isInFrame( const Vector3d& pt_world )
{
    ...
    if ( p_cam(2,0)<0 )
        return false;
    Vector2d pixel = camera_->world2camera(pt_world, T_c_w_);
